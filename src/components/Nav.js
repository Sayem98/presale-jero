...
export default function Nav() {
  ...
  const [ref_data, set_Ref_data] = useState(null);
  const [ref_data1, set_Ref_data1] = useState(null);
  ...
  async function mount() {
    try {
      ...
      let totalInvest = await contract.methods
        ...
        totalInvest = web3.utils.fromWei(totalInvest.toString(), "wei");

        let business = await contract.methods
        .totalbusiness()
        .call({ from: accounts[0].toString() });

        business = web3.utils.fromWei(business.toString(), "wei");
      ...
      const totalReferral = await contract.methods
        .DUSDTotalReferrals()
        .call({ from: accounts[0].toString() });
      balance = web3.utils.fromWei(balance, "nano")
      totalReward = Number(web3.utils.fromWei(totalReward, "ether")).toFixed(2);
